...
#1 tenant, who covers mortgage and pays small rent above as well as initial fee. 
...
A = matrix(nrow = 7, ncol= 360)
colnames(A) = paste("month", seq(1, ncol(A)))
catagories = c('total_assets','rent','tenentDepo', 'homeValue','remainingBal', 'profit_td-negHouseVal', 'profitTD_withHouseVal')
...
dfmod = data.frame('TotalValue' = A[1,], 'rent' = A[2,], 'tenantDepo' = A[3,], 'homeVal' = A[4,], 'remainingBal' = A[5,], 'profit_toDate_negHomeVal' = A[6,], 'profitTD_withHomeVal' = A[7,])
View(dfmod)
